...
function manifest(over: Partial<UpdateManifest> = {}): UpdateManifest {
  return {
    ...
    notes: '亮点',
    downloadUrl: 'https://example.com/Audelyra.dmg',
    mirrorUrl: null,
    ...over
  }
}
...
describe('决策记账（审修 C1：屏上按钮不能因一次失败检查变死键）', () => {
  ...
  it('canOpenUrl：只放行当前清单的主/镜像地址；无决策一律拒', () => {
    expect(canOpenUrl(active, 'https://example.com/Audelyra.dmg')).toBe(true)
    expect(canOpenUrl(active, 'https://evil.com/x.dmg')).toBe(false)
    expect(canOpenUrl(null, 'https://example.com/Audelyra.dmg')).toBe(false)
  })
