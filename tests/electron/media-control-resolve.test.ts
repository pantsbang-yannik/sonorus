...
describe('resolveBinary 打包环境候选', () => {
  ...
  const origEnv = process.env.AUDELYRA_MEDIA_CONTROL

  afterEach(() => {
    if (tmp) rmSync(tmp, { recursive: true, force: true })
    tmp = null
    proc.resourcesPath = origRes
    if (origEnv === undefined) delete process.env.AUDELYRA_MEDIA_CONTROL
    else process.env.AUDELYRA_MEDIA_CONTROL = origEnv
  })

  it('resourcesPath 下存在捆绑副本时优先返回它', () => {
    tmp = mkdtempSync(join(tmpdir(), 'audelyra-mc-'))
    ...
    delete process.env.AUDELYRA_MEDIA_CONTROL
    proc.resourcesPath = tmp
    expect(resolveBinary()).toBe(bin)
  })

  it('AUDELYRA_MEDIA_CONTROL 环境变量仍优先于捆绑副本', () => {
    tmp = mkdtempSync(join(tmpdir(), 'audelyra-mc-'))
    ...
    process.env.AUDELYRA_MEDIA_CONTROL = envBin
    proc.resourcesPath = tmp
